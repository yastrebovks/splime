...
def _runtime_box_pipeline() -> Pipeline:
    ...
    pipeline = lift_any(_runtime_consume_box).bind(
        box = producer).alias('consumer').render('runtime_pipeline')
    return cast(Pipeline, pipeline.add_adapter(
        RuntimeBox,
        'bytes',
...
def _format_pipeline(include_bytes: bool = True) -> Pipeline:
    ...
    csv_pipeline = lift_any(_format_consume_value).bind(
        value = producer.as_format('csv')).alias('csv').render()
    bytes_pipeline = lift_any(_format_consume_value).bind(
        value = producer.as_format('bytes')).alias('bytes').render()
    ...
    pipeline = pipeline.add_adapter(
        str,
        'csv',
        save = _format_save_csv,
        load = _format_load_csv)
    if not include_bytes:
        return pipeline
    return pipeline.add_adapter(
        str,
        'bytes',
        save = _format_save_bytes,
        load = _format_load_bytes)


def _plain_string_pipeline() -> Pipeline:
    lift_any = cast(Any, lift)
    producer = lift_any(_format_make_value)
    return cast(Pipeline, lift_any(_format_consume_value).bind(
        value = producer).alias('consumer').render('plain_string_pipeline'))
...
def _e2e_pipeline(module: Any) -> Pipeline:
    ...
    pipeline = lift_any(module.summarize_box).bind(
        box = normalizer).alias('summary').render('adapter_e2e_pipeline')
    return cast(Pipeline, pipeline.add_adapter(
        module.E2EBox,
        'txt',
        save = module.save_box,
        load = module.load_box))
...
def test_adapter_yaml_ir_round_trip_reconstructs_save_load(tmp_path: Path) -> None:
    ...
    [(root, dependencies)] = [
        (root, dependencies)
        for root, dependencies in get_top_level_deps(2, [adapter])
        if isinstance(root, DAdapter)]
    dumped = yaml.dump([root, *dependencies], sort_keys = False)
    [loaded_root, *loaded_dependencies] = yaml.load(dumped, Loader = SPLSafeLoader)

    reconstructed = _reconstruct_adapter(
        cast(DAdapter, loaded_root),
        cast(list[Any], loaded_dependencies))
...
def test_pipeline_or_merges_adapters_and_rejects_conflicts() -> None:
    left = Pipeline().add_adapter(
        str,
        'txt',
        save = _adapter_save,
        load = _adapter_load)
    right = Pipeline().add_adapter(
        bytes,
        'bin',
        save = _adapter_save,
        load = _adapter_load)
    ...
    assert set(merged.adapters) == {
        make_key(str, 'txt'),
        make_key(bytes, 'bin')}

    conflicting = Pipeline().add_adapter(
        str,
        'txt',
        save = _adapter_save_upper,
        load = _adapter_load)
...
def test_pipeline_merge_rejects_duplicate_adapter_key() -> None:
    ...
    with pytest.raises(
            ValueError,
            match = re.escape('pipeline adapter conflict: `{}`'.format(key))):
        left | right


def test_pipeline_yaml_round_trip_preserves_callable_adapters(tmp_path: Path) -> None:
    pipeline = Pipeline(name = 'adapter_pipeline').add_adapter(
        str,
        'txt',
        save = _adapter_save,
        load = _adapter_load)
    [(root, dependencies)] = [
        (root, dependencies)
        for root, dependencies in get_top_level_deps(2, [pipeline])
...
def test_builtin_adapter_requires_explicit_artifact_edge() -> None:
    deployment = cast(Any, Deployment)
    pipeline = _plain_string_pipeline().add_adapter(
        str,
        'csv',
        save = _format_save_csv,
        load = _format_load_csv)
...
def test_pipeline_builder_format_override_selects_edge_adapter() -> None:
    ...
    assert sorted([
        value.format
        for _, value in pipeline.links
        if isinstance(value, FormattedOutputRef)]) == ['bytes', 'csv']
    assert csv_result == {'default': 'csv:hello-format'}
    assert bytes_result == {'default': 'bytes:hello-format'}
    assert sorted([ref.key for ref in refs]) == [
        make_key(str, 'bytes'),
        make_key(str, 'csv')]
...
def test_artifact_edge_with_no_resolvable_adapter_raises_precise_error() -> None:
    ...
    producer = lift_any(_runtime_make_box)
    pipeline = lift_any(_runtime_consume_box).bind(
        box = producer.as_format('bytes')).alias('consumer').render('missing_adapter_pipeline')
    consumer = cast(Any, pipeline).get_node_by_alias('consumer')
    run = deployment(pipeline).run()

    with pytest.raises(
            ValueError,
            match = r'pipeline adapter is not found .*format `bytes`'):
        run[consumer]
...
def test_pipeline_builder_format_override_survives_yaml_round_trip(tmp_path: Path) -> None:
    ...
    assert sorted([
        ...
        if isinstance(link_to, DFormattedOutputRef)]) == ['bytes', 'csv']
    assert sorted([
        value.format
        for _, value in imported.links
        if isinstance(value, FormattedOutputRef)]) == ['bytes', 'csv']
    assert csv_result == {'default': 'csv:hello-format'}
    assert bytes_result == {'default': 'bytes:hello-format'}
