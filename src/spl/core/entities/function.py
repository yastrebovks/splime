...
yaml.add_representer(
    _body, lambda dumper, data: dumper.represent_scalar("tag:yaml.org,2002:str", data.value, style="|")
)
...
class DFunction(DBase):
    ...
    def __hash__(self) -> int:
        return hash((self.name, self.body, tuple(self.inputs), tuple(self.outputs or ())))
...
yaml.add_representer(
    ...
    lambda dumper, data: dumper.represent_mapping(
        ...
        {
            "name": data.name,
            "inputs": list(map(lambda x: {"name": x.name, "type": x.typ_, "default": x.default}, data.inputs)),
            "outputs": None
            if data.outputs is None
            else list(map(lambda x: {"name": x.name, "type": x.typ_}, data.outputs)),
...
yaml.add_constructor(
    "!DFunction",
    lambda loader, node: (
        lambda data: DFunction(
            name=data["name"],
            body=data["body"],
            inputs=[InputPort(name=x["name"], typ_=x["type"], default=x.get("default")) for x in data["inputs"]],
            outputs=[OutputPort(name=x["name"], typ_=x["type"]) for x in data["outputs"]],
        )
    )(cast(dict[str, Any], loader.construct_mapping(cast(Any, node), deep=True))),
)
...
def get_dependencies_from_bytecode(frame_offset: int, f: FunctionType | CodeType | str) -> Generator[Any]:
    ...
    names = sorted(filter(lambda x: x not in vars(builtins), set(get_dependency_names_from_bytecode(f))))
...
def get_dependencies_from_ast(frame_offset: int, tree: ast.FunctionDef) -> Generator[Any]:
...
def serialize_function_output(func: FunctionType, tree: ast.FunctionDef) -> list[OutputPort]:
    ...
    return_type = cast(Any, func.__annotations__).get("return")
...
def serialize_function(func: FunctionType, tree: ast.FunctionDef | None = None) -> DFunction:
    ...
    return DFunction(
        name=tree.name,
        body=ast.unparse(cast(ast.AST, tree.body)),
        inputs=[
            InputPort(
                name=a.arg,
            ...
            for a, d in zip(args.args, [*[None] * (len(args.args) - len(args.defaults)), *args.defaults], strict=True)
        ],
        outputs=serialize_function_output(func, tree),
    )


@ir_parse.register(lambda x: isinstance(x, FunctionType) and (getattr(x, "__module__", None) == "__main__"))
def _ir_parse__function(x: FunctionType, name: str | None = None) -> _attach | _branch:
    ...
    [node] = ast.parse(textwrap.dedent(inspect.getsource(x))).body
    tree = cast(ast.FunctionDef, node)
    return _branch(
        x,
        lambda: serialize_function(x, tree),
        lambda frame_offset: chain(
            get_dependencies_from_bytecode(frame_offset, x), get_dependencies_from_ast(frame_offset, tree)
        ),
    )


def get_function_metadata(func: FunctionType) -> DFunction:
    if hasattr(func, METADATA_DUNDER_NAME):
        # We imported this function using SPL, using it's metadata.
        return cast(DFunction, getattr(func, METADATA_DUNDER_NAME))
...
def _ir_unparse__function(x: DFunction, source: Path) -> Generator[ast.stmt]:
    ...
    yield ast.FunctionDef(
        ...
        args=ast.arguments(
            args=[
                ast.arg(
                    arg=port.name, annotation=ast.parse(port.typ_, mode="eval").body if port.typ_ is not None else None
                )
                for port in x.inputs
            ],
            defaults=[ast.parse(port.default, mode="eval").body for port in x.inputs if port.default is not None],
        ),
    )

    # Importing helpers
    yield ast.ImportFrom(
        module=m_node.__name__, names=[ast.alias(name="InputPort"), ast.alias(name="OutputPort")], level=0
    )

    yield ast.ImportFrom(module=__name__, names=[ast.alias(name="DFunction")], level=0)
    ...
    yield ast.Expr(
        value=ast.Call(
            ...
            args=[
                ...
                ast.Tuple(elts=[ast.Constant(value=str(source.absolute())), ast.Constant(value=x.name)]),
            ],
        )
    )
                                ...
                                elts=[
                                    ast.Call(
                                        ...
                                        keywords=[
                                            ...
                                            ast.keyword(arg="default", value=ast.Constant(value=port.default)),
                                        ],
                                    )
                                    for port in x.inputs
                                ],
