...
class NodeFunction(Node):
    ...
    def __init__(self, func, uuid: UUID | None = None):
...
yaml.add_representer(
    DNodeFunction,
    lambda dumper, data: dumper.represent_mapping('!DNodeFunction', data.__dict__))

yaml.add_constructor(
    '!DNodeFunction',
    lambda loader, node: DNodeFunction(**loader.construct_mapping(node)))


@ir_parse.register(
    lambda x: isinstance(x, NodeFunction))
def _ir_parse__node_function(
        x: NodeFunction,
        name: str | None = None):

    return _branch(
        x,
        lambda: DNodeFunction(
            uuid = str(x.uuid),
            func = get_function_metadata(x.func).name),
        lambda frame_offset: [ir_parse(x.func, name = name)])


@ir_unparse.register(
    lambda x: isinstance(x, DNodeFunction))
    ...
    yield ast.Assign(
        ...
        value = ast.Call(
            ...
            keywords = [
                ...
                ast.keyword(
                    arg = 'func',
                    value = ast.Name(id = x.func, ctx = ast.Load()))]))
