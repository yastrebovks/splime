...
class Pipeline:
    ...
    def __or__(self, other: "Pipeline") -> "Pipeline":
        ...
        return Pipeline(nodes=nodes, links=links, aliases=aliases, adapters=adapters)._validate_consistency()
    ...
    def add_link(self, node_input_ref: NodeInputRef, value: Any) -> "Pipeline":
        ...
        return Pipeline(
            nodes=self.nodes, links={*self.links, (node_input_ref, value)}, aliases=self.aliases, adapters=self.adapters
...
yaml.add_representer(DPipeline, lambda dumper, data: dumper.represent_mapping("!DPipeline", data.__dict__))
    ...
    yield ast.Assign(
        ...
        value=ast.Call(
            ...
            keywords=[
                ...
                ast.keyword(
                    arg="links", value=ast.Set(elts=[ast.Starred(value=ast.Name(id="_links", ctx=ast.Load()))])
                ),
                ast.keyword(
                    arg="aliases",
                    value=ast.Dict(
