...
def encode(value: Any, adapter: Adapter, artifacts_dir: Path) -> ArtifactRef:
    ...
    return ArtifactRef(key=adapter.key, uri=str(artifact_path), sha256=sha256, size=size)
...
class Deployment:
    def __init__(self, client: Any = None, pipeline: Pipeline | None = None) -> None:
        if pipeline is None:
            pipeline = client
            client = None
        self._client = client
...
class Run:
    def __init__(self, callback: Callable[..., dict[str, Any]], pipeline: Pipeline, **kwargs: Any) -> None:
    ...
    def _round_trip_artifact(
        ...
        adapter = self._pipeline.resolve_adapter(py_type=type(value), format=adapter_format)
        if adapter is None:
    ...
    def _get_input(self, x: Any) -> Any:
        match x:
            ...
            case NodeOutputRef():
                return self._round_trip_artifact((self._get_result(x.node))[x.port.name], x)
    ...
    def _get_result(self, node: Node) -> dict[str, Any]:
        if node not in self._results:
            ...
            if node in self._deps:
                kwargs = kwargs | {port: self._get_input(v) for port, v in self._deps[node].items()}
